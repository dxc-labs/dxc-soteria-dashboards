...
class CheckpointsDocumentation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // Code editor
  ...
  async componentDidMount() {
    ...
    const url = (`${process.env.REACT_APP_USER_DOMAIN}dashboards/documents/Checkpoints-README.md`);
    console.log(url);
    fetch(url)
        .then((res) => res.text())
        .then((md) => {
  ...
  updatefile() {
    ...
    const postdata = {
      ...
      body: JSON.stringify({data: this.state.outputText, file: 'Checkpoints-README.md'}),
    };
...
export default CheckpointsDocumentation;
