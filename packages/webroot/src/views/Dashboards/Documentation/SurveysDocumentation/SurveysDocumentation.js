...
class SurveysDocumentation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // Code editor
  ...
  async componentDidMount() {
    ...
    const url = (`${process.env.REACT_APP_USER_DOMAIN}dashboards/documents/Surveys-README.md`);
    await fetch(url)
        .then((res) => res.text())
        .then((md) => {
  ...
  updatefile() {
    ...
    const postdata = {
      ...
      body: JSON.stringify({data: this.state.outputText, file: 'Surveys-README.md'}),

    };
    fetch(`${process.env.REACT_APP_DASHBOARD_API}dashboards/updatefile`, postdata)
...
export default SurveysDocumentation;
