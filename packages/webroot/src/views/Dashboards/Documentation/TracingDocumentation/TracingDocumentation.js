...
class TracingDocumentation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      // Code editor
  ...
  async componentDidMount() {
    ...
    const url = (`${process.env.REACT_APP_USER_DOMAIN}dashboards/documents/Tracing-README.md`);
    await fetch(url)
        .then((res) => res.text())
        .then((md) => {
  ...
  updatefile() {
    ...
    const postdata = {
      ...
      body: JSON.stringify({data: this.state.outputText, file: 'Tracing-README.md'}),
...
export default TracingDocumentation;
